...
# Save categorical and numeric columns for later use
joblib.dump(categorical_cols, os.path.join(data_dir, 'categorical_cols_nn.joblib'))
joblib.dump(list(X_train.drop(columns=categorical_cols).columns), os.path.join(data_dir, 'numeric_cols_nn.joblib'))
...
model.compile(optimizer=optimizer, loss='mse', metrics=['mae'])  # type: ignore
